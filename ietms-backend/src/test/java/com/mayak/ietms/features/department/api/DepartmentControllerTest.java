...
public class DepartmentControllerTest {
    ...
    static RequestPostProcessor asAdmin() {
        return authentication(new UsernamePasswordAuthenticationToken(
                1L, null, List.of(new SimpleGrantedAuthority("MANAGE_DEPARTMENTS"))));
    }
    ...
    @Test
    @DisplayName("GET /api/departments — no auth → 401")
    ...
    @Test
    @DisplayName("POST /api/departments — no auth → 401")
    public void create_noAuth_returns401() throws Exception {
        mockMvc.perform(post("/api/departments")).andExpect(status().isUnauthorized());
    }

    @Test
    @DisplayName("DELETE /api/departments/{id} — no auth → 401")
    public void delete_noAuth_returns401() throws Exception {
        mockMvc.perform(delete("/api/departments/1")).andExpect(status().isUnauthorized());
    }

    // ─────────────────────────────────────────────────────────────
    // 403 — authenticated, no MANAGE_DEPARTMENTS
    // ─────────────────────────────────────────────────────────────

    @Test
    @DisplayName("POST /api/departments — no MANAGE_DEPARTMENTS → 403")
    public void create_noAuthority_returns403() throws Exception {
        mockMvc.perform(post("/api/departments").with(asUser())
                        .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("PUT /api/departments/{id} — no MANAGE_DEPARTMENTS → 403")
    public void update_noAuthority_returns403() throws Exception {
        mockMvc.perform(put("/api/departments/1").with(asUser())
                        .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isForbidden());
    }

    @Test
    @DisplayName("DELETE /api/departments/{id} — no MANAGE_DEPARTMENTS → 403")
    public void delete_noAuthority_returns403() throws Exception {
        mockMvc.perform(delete("/api/departments/1").with(asUser()))
                .andExpect(status().isForbidden());
    }

    // ─────────────────────────────────────────────────────────────
    // Happy path — authenticated / MANAGE_DEPARTMENTS
    // ─────────────────────────────────────────────────────────────

    @Test
    @DisplayName("GET /api/departments — authenticated → 200")
    public void findAll_authenticated_returns200() throws Exception {
        mockMvc.perform(get("/api/departments").with(asUser())).andExpect(status().isOk());
    }

    @Test
    @DisplayName("POST /api/departments — MANAGE_DEPARTMENTS → 200")
    public void create_withAuthority_returns200() throws Exception {
        mockMvc.perform(post("/api/departments").with(asAdmin())
                        .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isOk());
    }

    @Test
    @DisplayName("PUT /api/departments/{id} — MANAGE_DEPARTMENTS → 200")
    public void update_withAuthority_returns200() throws Exception {
        mockMvc.perform(put("/api/departments/1").with(asAdmin())
                        .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isOk());
    }

    @Test
    @DisplayName("DELETE /api/departments/{id} — MANAGE_DEPARTMENTS → 200")
    public void delete_withAuthority_returns200() throws Exception {
        mockMvc.perform(delete("/api/departments/1").with(asAdmin()))
                .andExpect(status().isOk());
    }
}